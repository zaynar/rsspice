...
//$Procedure T_PCKEQD ( Text PCK equation implementation, 2017 draft )
pub fn T_PCKEQD(
    BODY: i32,
    ET: f64,
    RA: f64,
    ...
    //
    spicelib::CHKIN(b"T_PCKEQD", ctx)?;
    spicelib::SIGERR(b"SPICE(BOGUSENTRY)", ctx)?;
    spicelib::CHKOUT(b"T_PCKEQD", ctx)?;
    Ok(())
}

//$Procedure T_PCKASTD ( Text PCK Euler angle body set )
pub fn T_PCKASTD(ANGSET: &mut [i32], ctx: &mut Context) -> f2rust_std::Result<()> {
    let mut ANGSET = DummyArrayMut::new(ANGSET, LBCELL..);

    spicelib::CHKIN(b"T_PCKASTD", ctx)?;
    ...
    // 802 is Nereid.
    //
    spicelib::CHKOUT(b"T_PCKASTD", ctx)?;
    Ok(())
}

//$Procedure T_PCKRSTD ( Text PCK radii body set )
pub fn T_PCKRSTD(RADSET: &mut [i32], ctx: &mut Context) -> f2rust_std::Result<()> {
    let mut RADSET = DummyArrayMut::new(RADSET, LBCELL..);

    spicelib::CHKIN(b"T_PCKRSTD", ctx)?;
    ...
    spicelib::CHKOUT(b"T_PCKRSTD", ctx)?;
    Ok(())
}

//$Procedure T_PCKANGD ( Text PCK Euler angle equation implementation )
pub fn T_PCKANGD(
    BODY: i32,
    ET: f64,
    RA: &mut f64,
    ...
    spicelib::CHKIN(b"T_PCKANGD", ctx)?;
    ...
    if (BODY == 10) {
        ...
        M5 = (((189.6327156 + (41215158.1842005 * T)) + (12.71192322 * f64::powi(T, 2)))
            * spicelib::RPD(ctx));
        //
        // M5 quadratic term: 12.71192322 * T**2
        //
        M6 = (((19.26538605 + (82430316.3686428 * T)) + (25.42412173 * f64::powi(T, 2)))
            * spicelib::RPD(ctx));
        //
        // M6 quadratic term: 25.42412173 * T**2
        //
        M7 = (((208.89882434 + (123645474.5446679 * T)) + (38.13293168 * f64::powi(T, 2)))
            * spicelib::RPD(ctx));
        //
        ...
        if (BODY == 401) {
            ...
            spicelib::CHKOUT(b"T_PCKANGD", ctx)?;
            return Ok(());
        }
    } else if ((BODY / 100) == 5) {
...
//$Procedure T_PCKRADD ( Text PCK body radii )
pub fn T_PCKRADD(BODY: i32, RADII: &mut [f64], ctx: &mut Context) -> f2rust_std::Result<()> {
    let mut RADII = DummyArrayMut::new(RADII, 1..=3);

    spicelib::CHKIN(b"T_PCKRADD", ctx)?;
    ...
    if (BODY == 10) {
        ...
        spicelib::ERRINT(b"#", BODY, ctx);
        spicelib::SIGERR(b"SPICE(INVALIDBODYCODE)", ctx)?;
        spicelib::CHKOUT(b"T_PCKRADD", ctx)?;
        return Ok(());
    }
