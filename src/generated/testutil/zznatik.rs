...
pub fn ZZNATIK(NAMEIK: &[u8], SPK: &[u8], PCK: &[u8], ctx: &mut Context) -> f2rust_std::Result<()> {
    let mut IK = ActualCharArray::new(LNSIZE, 1..=NLINES);
    ...
    //
    L = 1;
    fstr::assign(IK.get_mut(L), b" ");
    L = (L + 1);
    fstr::assign(IK.get_mut(L), b" KPL/IK");
    L = (L + 1);
    fstr::assign(IK.get_mut(L), b" ");
    L = (L + 1);
    BEGDAT(&mut IK[L]);
    ...
    fstr::assign(IK.get_mut(L), b"NAIF_BODY_NAME += \'#\' ");
    spicelib::REPMC(&IK[L].to_vec(), b"#", &INSNAM, &mut IK[L]);
    L = (L + 1);

    fstr::assign(IK.get_mut(L), b"NAIF_BODY_CODE += #");
    spicelib::REPMI(&IK[L].to_vec(), b"#", INST, &mut IK[L], ctx);
    L = (L + 1);
    ...
    IXINB = L;
    fstr::assign(IK.get_mut(L), b"INS#_FOV_SHAPE            = \'ELLIPSE\' ");
    L = (L + 1);
    fstr::assign(
        IK.get_mut(L),
    ...
    L = (L + 1);
    IXBSB = L;
    fstr::assign(IK.get_mut(L), b"INS#_BORESIGHT            = ( *");
    L = (L + 1);
    fstr::assign(IK.get_mut(L), b"                                     *");
    L = (L + 1);
    fstr::assign(IK.get_mut(L), b"                                     * )");
    L = (L + 1);
    IXBCB = L;
    fstr::assign(IK.get_mut(L), b"INS#_FOV_BOUNDARY         = ( *");
    IXINE = L;
    L = (L + 1);
    fstr::assign(IK.get_mut(L), b"                                     *");
    L = (L + 1);
    ...
    for I in IXINB..=IXINE {
        spicelib::REPMI(&IK[I].to_vec(), b"#", INST, &mut IK[I], ctx);
    }
    ...
    IXINB = L;
    fstr::assign(IK.get_mut(L), b"INS#_FOV_SHAPE            = \'CIRCLE\' ");
    L = (L + 1);
    fstr::assign(
        IK.get_mut(L),
        b"INS#_FOV_FRAME            = \'ALPHA_VIEW_XY\' ",
    ...
    fstr::assign(IK.get_mut(L), b"INS#_FOV_SHAPE            = \'RECTANGLE\' ");
    L = (L + 1);
    fstr::assign(
        IK.get_mut(L),
        b"INS#_FOV_FRAME            = \'ALPHA_VIEW_XY\' ",
    ...
    fstr::assign(IK.get_mut(L), b"INS#_FOV_SHAPE            = \'POLYGON\' ");
    L = (L + 1);
    fstr::assign(
        IK.get_mut(L),
    ...
    spicelib::UNLOAD(PCK, ctx)?;
    spicelib::UNLOAD(SPK, ctx)?;
    ...
    spicelib::TXTOPN(NAMEIK, &mut UNIT, ctx)?;

    for I in 1..=NL {
        spicelib::WRITLN(&IK[I], UNIT, ctx)?;
    }
