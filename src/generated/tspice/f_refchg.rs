...
pub fn F_REFCHG(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    let mut TPMI = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut TROTAT = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut ROTAT = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut MODIFY = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut EXPECT = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut TK2CK = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut TK2IN = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut TK2J2 = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut CK2IN = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut EARTH: i32 = 0;
    let mut CENT: i32 = 0;
    let mut CLASS: i32 = 0;
    ...
    fstr::assign(LINES.get_mut(4), b"by calling the test utility TSTCKN.");
    ...
    spicelib::REFCHG(-222222, 1, ET, ROTAT.as_slice_mut(), ctx)?;
    ...
    spicelib::REFCHG(27687628, -1987291, ET, ROTAT.as_slice_mut(), ctx)?;
    ...
    for I in 1..=18 {
        {
            ...
            for _ in 0..((m2__ - m1__ + m3__) / m3__) as i32 {
                spicelib::REFCHG(I, J, ET, ROTAT.as_slice_mut(), ctx)?;
    ...
    for I in 10010..=10027 {
        ...
        spicelib::TIPBOD(b"J2000", CLSSID, ET, TIPM.as_slice_mut(), ctx)?;
        spicelib::REFCHG(1, J, ET, ROTAT.as_slice_mut(), ctx)?;
    ...
    for I in 10010..=10027 {
        ...
        spicelib::FRINFO(J, &mut CENT, &mut CLASS, &mut CLSSID, &mut FOUND, ctx)?;
        spicelib::TIPBOD(b"J2000", CLSSID, ET, TIPM.as_slice_mut(), ctx)?;
        spicelib::XPOSE(TIPM.as_slice(), TPMI.as_slice_mut());
        spicelib::REFCHG(J, 1, ET, ROTAT.as_slice_mut(), ctx)?;
    ...
    spicelib::XPOSE(ROT.as_slice(), TROTAT.as_slice_mut());
    ...
    for I in 1..=3 {
        ...
        spicelib::REFCHG(ID, 1, ET, ROTAT.as_slice_mut(), ctx)?;
        //
        ...
        //
        spicelib::REFCHG(XPCTD[I], 1, ET, MODIFY.as_slice_mut(), ctx)?;
        spicelib::MXM(MODIFY.as_slice(), TROTAT.as_slice(), EXPECT.as_slice_mut());
    ...
    spicelib::REFCHG(-111111, EARTH, ET, ROTAT.as_slice_mut(), ctx)?;
    ...
    spicelib::REFCHG(XPCTD[3], 1, ET, MODIFY.as_slice_mut(), ctx)?;
    ...
    //
    testutil::TSTATD(ET, ROT.as_slice_mut(), AV.as_slice_mut(), ctx);
    spicelib::XPOSE(ROT.as_slice(), CK2IN.as_slice_mut());

    spicelib::TIPBOD(b"J2000", 399, ET, TIPM.as_slice_mut(), ctx)?;

    spicelib::MXM(CK2IN.as_slice(), TK2CK.as_slice(), TK2IN.as_slice_mut());
    spicelib::MXM(MODIFY.as_slice(), TK2IN.as_slice(), TK2J2.as_slice_mut());
    spicelib::MXM(TIPM.as_slice(), TK2J2.as_slice(), TROTAT.as_slice_mut());
    ...
    spicelib::XPOSE(ROT.as_slice(), MODIFY.as_slice_mut());

    spicelib::MXM(MODIFY.as_slice(), TROTAT.as_slice(), EXPECT.as_slice_mut());

    spicelib::REFCHG(-111111, -399999, ET, ROTAT.as_slice_mut(), ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
