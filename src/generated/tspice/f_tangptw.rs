...
//$Procedure F_TANGPTW ( Test tangent point routine, wrapper tests )
pub fn F_TANGPTW(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    //
    testutil::TOPEN(b"F_TANGPTW", ctx)?;
    ...
    // --- Case: -------------------------------------------------------
    //
    testutil::TCASE(b"Simple case 1: Earth observes Moon, abcorr = NONE. Ray points toward a point 100 km above the Moon\'s north pole. Use tangent point locus.", ctx)?;
    ...
    // --- Case: ----------------------------------------------------------
    //
    testutil::TCASE(b"Simple case 2: Phobos observes Mars, abcorr = CN+S. Ray points toward a point 350 km above Mars\'s north pole. Use tangent point locus.", ctx)?;
    ...
    // --- Case: -----------------------------------------------------------
    //
    testutil::TCASE(b"Simple case 3: Phobos observes Mars, abcorr = XCN+S. Ray points toward a point 350 km above Mars\'s north pole. Use surface point locus.", ctx)?;
    ...
    //
    // Check SRFVEC. In order to compute an expected vector, we can
    // simply treat the surface point as an ephemeris object.
    //
    // Find the aberration-corrected position of the surface point
    // relative to the observer, expressed in the target body-fixed
    // frame.
    //
    spicelib::SPKCPT(
        save.SRFPT.as_slice(),
    ...
    //
    // This tolerance value was chosen to support the
    // PC/Windows/Intel Fortran platform. The previous value was 1.D-12.
    //
    save.TOL = 0.000000000002;
    ...
    //
    // Compute the position of the observer relative to the target
    // center in the body-fixed frame.
    //
    spicelib::VSUB(
        save.SRFPT.as_slice(),
        save.SRFVEC.as_slice(),
    ...
    save.SEP = spicelib::VSEP(save.TANVEC.as_slice(), save.DVEC.as_slice(), ctx);
    ...
    fstr::assign(&mut save.CORLOC, b"XYZ");
