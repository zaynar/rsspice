...
//$Procedure F_NNEK01 ( EK test, subset 1, non-native )
pub fn F_NNEK01(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    //
    // Create an EK. Convert to non-native format.
    //
    save.FILENO = 1;

    testutil::TSTEK(EK1NAT, save.FILENO, MAXROW, false, &mut save.EK1HAN, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;

    T_BINGO(EK1NAT, EK1, save.NNBFF, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
    ...
    //
    // EK1 wil be deleted by the test system.
    //
