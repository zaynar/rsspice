...
const SPK12E: &[u8] = b"test12err.bsp";
const SPK12: &[u8] = b"test12.bsp";
const SPK12B: &[u8] = b"test12big.bsp";
const SPK12S: &[u8] = b"test12sub.bsp";
...
pub fn F_SPK12(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: bad frame name.", ctx)?;
    ...
    //
    fstr::assign(&mut SEGID, b"SPK type 12 test segment");

    //
    // Open a new SPK file.
    //
    if spicelib::EXISTS(SPK12E, ctx)? {
        spicelib::DELFIL(SPK12E, ctx)?;
    }

    spicelib::SPKOPN(
        SPK12E,
        b"Type 12 SPK internal file name",
        4,
        &mut HANDLE,
        ctx,
    )?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: SEGID too long.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: unprintable SEGID characters.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: polynomial degree too high.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: polynomial degree too low.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: even polynomial degree.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: too few states", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: descriptor times swapped.", ctx)?;
    ...
    //
    testutil::TCASE(
        b"SPKW12 error case: descriptor start time is too early.",
        ctx,
    )?;
    ...
    spicelib::SPKW12(
        ...
        NDSCRT,
        save.DSCSTS.as_slice(),
        (save.DSCEPC[1] * (1.0 + 0.00000000000001)),
        STEP,
        ctx,
    )?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: descriptor end time is too late.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW12 error case: bad step size.", ctx)?;
    ...
    testutil::TCASE(b"Create a type 12 segment.", ctx)?;
    ...
    // Load the SPK file.
    //
    spicelib::SPKLEF(SPK12, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
    ...
    //
    testutil::TCASE(
        b"SPKW12 test:  create a large segment with multiple directories.",
        ctx,
    )?;
    ...
    //
    if spicelib::EXISTS(SPK12B, ctx)? {
        spicelib::DELFIL(SPK12B, ctx)?;
    }
    ...
    // Load the SPK file.
    //
    spicelib::SPKLEF(SPK12B, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"Test SPKS12: write new file having a segment created by subsetting small segment from SPK12.", ctx)?;
    ...
    // Create a segment identifier.
    //
    fstr::assign(&mut SEGID, b"SPK type 12 test subset segment");

    //
    // Open a new SPK file.
    //
    if spicelib::EXISTS(SPK12S, ctx)? {
        spicelib::DELFIL(SPK12S, ctx)?;
    }

    spicelib::SPKOPN(SPK12S, b"Type 12 SPK internal file name", 0, &mut NEWH, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;

    //
    // Open SPK12 and extract segment descriptor and ID of first segment.
    //
    spicelib::DAFOPR(SPK12, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
    ...
    //
    testutil::TCASE(
        b"Test SPKS12: check descriptor bounds on subsetted file.",
        ctx,
    )?;
    ...
    // segment.
    //
    spicelib::DAFOPR(SPK12S, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
    ...
    // Load the SPK file.
    //
    spicelib::SPKLEF(SPK12S, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
