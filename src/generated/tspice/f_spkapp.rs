...
pub fn F_SPKAPP(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    testutil::TCASE(
        b"Compare SPKAPP and SPKGEO when no corrections are requested ",
        ctx,
    )?;
    ...
    testutil::TCASE(
        b"Compare SPKAPP and SPKAPP_O ( the old version ) when light time only is requested. ",
        ctx,
    )?;
    ...
    testutil::TCASE(b"Compare SPKAPP and SPKAPP_O when light time plus stellar aberration corrections are applied. ", ctx)?;
    ...
    testutil::TCASE(b"Repeat for the transmission case.", ctx)?;
    ...
    testutil::TCASE(
        b"Repeat, using blanks and mixed case for REF, where REF is inertial",
        ctx,
    )?;
    ...
    spicelib::VMINUG(STATE.as_slice(), 6, STATE2.as_slice_mut());
    ...
    testutil::TCASE(b"Test \"CN\" transmit stellar aberration:  make sure correction is same as that obtained by using CN only and then applying STLABX. ", ctx)?;
