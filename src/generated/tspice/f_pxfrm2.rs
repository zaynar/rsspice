...
//$Procedure      F_PXFRM2 ( Family of tests for PXFRM2 )
pub fn F_PXFRM2(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    let mut ETFROM: f64 = 0.0;
    let mut ETTO: f64 = 0.0;
    ...
    //
    testutil::TCASE(
        b"Verify that both input and output frame names must be recognized. ",
        ctx,
    )?;
    ...
    spicelib::PXFRM2(b"SPUD", b"SPAM", ETFROM, ETTO, XFORM.as_slice_mut(), ctx)?;
    ...
    //
    testutil::TCASE(
        b"Verify that the input frame name must be recognized. ",
        ctx,
    )?;
    ...
    //
    testutil::TCASE(
        b"Verify that the output frame name must be recognized. ",
        ctx,
    )?;
    ...
    //
    testutil::TCASE(
        b"Make sure that an inertial frame transformation behaves as expected. ",
        ctx,
    )?;
    ...
    // Retrieve the names of the reference frames
    ...
    spicelib::PXFRM2(&NAMEI, &NAMEJ, ETFROM, ETTO, XFORM.as_slice_mut(), ctx)?;
...

    {
        ...
        for _ in 0..((m2__ - m1__ + m3__) / m3__) as i32 {
            ...
            spicelib::PXFRM2(b"J2000", &NAME[I], ETFROM, ETTO, XFORM.as_slice_mut(), ctx)?;

            // The subroutine TIPBOD returns a rotation matrix that
            // transforms from inertial to body-fixed coordinates.
            // Two separate calls are not necessary to test this with
            // PXFRM2 because the 'FROM' frame is inertial.
            spicelib::TIPBOD(b"J2000", IDCODE[I], ETTO, TSIPM.as_slice_mut(), ctx)?;
    ...
    //
    testutil::TCASE(
        b"Make sure the transformation from body-fixed to J2000 works. ",
        ctx,
    )?;

    {
        ...
        for _ in 0..((m2__ - m1__ + m3__) / m3__) as i32 {
            ...
            spicelib::PXFRM2(&NAME[I], b"J2000", ETFROM, ETTO, XFORM.as_slice_mut(), ctx)?;
            spicelib::TIPBOD(b"J2000", IDCODE[I], ETFROM, TSIPM.as_slice_mut(), ctx)?;
    ...
    //
    testutil::TCASE(
        b"Make sure that we can transform from one bodyfixed frame to another. ",
        ctx,
    )?;
    ...
    spicelib::TIPBOD(b"J2000", IDCODE[I], ETFROM, TSPMI.as_slice_mut(), ctx)?;
    spicelib::XPOSE(TSPMI.as_slice(), TMPMAT.as_slice_mut());
    spicelib::MOVED(TMPMAT.as_slice(), 9, TSPMI.as_slice_mut());

    spicelib::TIPBOD(b"J2000", IDCODE[J], ETTO, TSIPM.as_slice_mut(), ctx)?;

    spicelib::MXM(TSIPM.as_slice(), TSPMI.as_slice(), EFORM.as_slice_mut());

    spicelib::PXFRM2(&NAME[I], &NAME[J], ETFROM, ETTO, XFORM.as_slice_mut(), ctx)?;
    ...
    //
    testutil::TCASE(
        b"Make sure a long transformation chain is properly computed. ",
        ctx,
    )?;
    ...
    spicelib::REFCHG(TOPO, 1, ETFROM, TSPMI.as_slice_mut(), ctx)?;
    spicelib::REFCHG(1, PHNX, ETTO, TSIPM.as_slice_mut(), ctx)?;
    spicelib::MXM(TSIPM.as_slice(), TSPMI.as_slice(), EFORM.as_slice_mut());
    ...
    //
    testutil::TCASE(
        b"Verify the result from PXFRM2 is equal to PXFORM when ETFROM and ETTO are equal.",
        ctx,
    )?;
