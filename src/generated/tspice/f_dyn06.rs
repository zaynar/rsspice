...
pub fn F_DYN06(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    let mut ECLEQN = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    let mut ET: f64 = 0.0;
    let mut MOB: f64 = 0.0;
    let mut NRMERR: f64 = 0.0;
    let mut OBLMAT = StackArray2D::<f64, 9>::new(1..=3, 1..=3);
    ...
    for CASE in 1..=NCART {
        ...
        if GO {
            ...
            //
            fstr::assign(&mut FRNAME, b"MEAN_ECL_OF_DATE");
            FRCODE = -2000000000;
            ...
            fstr::assign(
                ...
                &fstr::concat(
                    &fstr::concat(
                        &fstr::concat(&fstr::concat(&FRSTEM, KWOBQM), b" = \'"),
                        KVM003,
                    ),
                    b"\'",
                ),
            ...
            // Pick a time value.
            //
            ET = (1000000.0 * (CASE - (NCART / 2)) as f64);
            ...
            //
            if spicelib::EQSTR(&RSTSTR, b"ROTATING") {
                //
            ...
            } else if spicelib::EQSTR(&RSTSTR, b"INERTIAL") {
                //
            ...
            //
            spicelib::ROTATE(MOB, 1, OBLMAT.as_slice_mut(), ctx);
            testutil::CHCKXC(false, b" ", OK, ctx)?;

            //
            // Form the J2000 to mean-ECL-of-date matrix.
            //
            spicelib::MXM(OBLMAT.as_slice(), PRECM.as_slice(), ECLEQN.as_slice_mut());
            ...
            testutil::CHCKXC(false, b" ", OK, ctx)?;
            spicelib::MXMT(RJ2B.as_slice(), ECLEQN.as_slice(), R2.as_slice_mut());
