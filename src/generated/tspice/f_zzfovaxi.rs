...
//$Procedure      F_ZZFOVAXI ( Test ZZFOVAXI )
pub fn F_ZZFOVAXI(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    //
    testutil::TOPEN(b"F_ZZFOVAXI", ctx)?;
...

    {
        ...
        for _ in 0..((m2__ - m1__ + m3__) / m3__) as i32 {
            ...
            // should be very close to the average of the unit boundary
            // vectors.
            //
            if spicelib::EVEN(save.N) {
                ...
                for I in 1..=save.N {
                    spicelib::VHAT(save.BOUNDS.subarray([1, I]), save.UVEC.as_slice_mut());
                    spicelib::VADD(
                        save.SUM.as_slice(),
                        save.UVEC.as_slice(),
                        save.V.as_slice_mut(),
                ...
                spicelib::VHATIP(save.CENTER.as_slice_mut());
                testutil::CHCKXC(false, b" ", OK, ctx)?;
