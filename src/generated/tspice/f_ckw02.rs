...
pub fn F_CKW02(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    let mut STOP = StackArray::<f64, 4>::new(1..=COUNT);
    ...
    //
    testutil::TOPEN(b"F_CKW02", ctx)?;

    testutil::KILFIL(b"ckw02.bc", ctx)?;
    spicelib::CKOPN(b"ckw02.bc", b"TestCK02", 1000, &mut HANDLE, ctx)?;
    ...
    testutil::TCASE(b"Case 1 with degenerate first interval.", ctx)?;
    ...
    testutil::TCASE(b"Case 2 with degenerate first interval.", ctx)?;
    ...
    testutil::TCASE(b"Case 1 with degenerate interval.", ctx)?;
    ...
    testutil::TCASE(b"Case 2 with degenerate interval.", ctx)?;
    ...
    testutil::TCASE(b"Case 1 with times out of order.", ctx)?;
    ...
    testutil::TCASE(b"Case 2 with times out of order.", ctx)?;
    ...
    testutil::TCASE(b"Case 2 with bad stop time.", ctx)?;
    ...
    spicelib::CKLPF(b"ckw02.bc", &mut HANDLE, ctx)?;
