...
pub fn T_CRDAF(
    ...
    spicelib::CHKIN(b"T_CRDAF", ctx)?;

    spicelib::LJUST(TYPE, &mut LOCTYP);
    spicelib::UCASE(&LOCTYP.clone(), &mut LOCTYP, ctx);

    if fstr::eq(&LOCTYP, b"SPK") {
        spicelib::SPKOPN(NAME, b" ", 0, &mut HANDLE, ctx)?;
    } else if fstr::eq(&LOCTYP, b"CK") {
        spicelib::CKOPN(NAME, b" ", 0, &mut HANDLE, ctx)?;
    } else {
        spicelib::PCKOPN(NAME, b" ", 0, &mut HANDLE, ctx)?;
    }
