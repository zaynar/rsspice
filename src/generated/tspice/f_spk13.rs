...
const SPK13E: &[u8] = b"test13err.bsp";
const SPK13: &[u8] = b"test13.bsp";
const SPK13B: &[u8] = b"test13big.bsp";
const SPK13S: &[u8] = b"test13sub.bsp";
...
pub fn F_SPK13(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW13 error case: bad frame name.", ctx)?;
    ...
    //
    fstr::assign(&mut SEGID, b"SPK type 13 test segment");

    //
    // Open a new SPK file.
    //
    if spicelib::EXISTS(SPK13E, ctx)? {
        spicelib::DELFIL(SPK13E, ctx)?;
    }

    spicelib::SPKOPN(
        SPK13E,
        b"Type 13 SPK internal file name",
        4,
        &mut HANDLE,
        ctx,
    )?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW13 error case: SEGID too long.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW13 error case: unprintable SEGID characters.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW13 error case: polynomial degree too high.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW13 error case: even polynomial degree.", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW13 error case: too few states", ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW13 error case: descriptor times swapped.", ctx)?;
    ...
    //
    testutil::TCASE(
        b"SPKW13 error case: descriptor start time is too early.",
        ctx,
    )?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"SPKW13 error case: descriptor end time is too late.", ctx)?;
    ...
    testutil::TCASE(b"Create a type 13 segment.", ctx)?;
    ...
    // Load the SPK file.
    //
    spicelib::SPKLEF(SPK13, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
    ...
    //
    testutil::TCASE(
        b"SPKW13 test:  create a large segment with multiple directories.",
        ctx,
    )?;
    ...
    //
    if spicelib::EXISTS(SPK13B, ctx)? {
        spicelib::DELFIL(SPK13B, ctx)?;
    }
    ...
    // Load the SPK file.
    //
    spicelib::SPKLEF(SPK13B, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
    ...
    // --- Case: ------------------------------------------------------
    //
    testutil::TCASE(b"Test SPKS13: write new file having a segment created by subsetting small segment from SPK13.", ctx)?;
    ...
    // Create a segment identifier.
    //
    fstr::assign(&mut SEGID, b"SPK type 13 test subset segment");

    //
    // Open a new SPK file.
    //
    if spicelib::EXISTS(SPK13S, ctx)? {
        spicelib::DELFIL(SPK13S, ctx)?;
    }

    spicelib::SPKOPN(SPK13S, b"Type 13 SPK internal file name", 0, &mut NEWH, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;

    //
    // Open SPK13 and extract segment descriptor and ID of first segment.
    //
    spicelib::DAFOPR(SPK13, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
    ...
    //
    testutil::TCASE(
        b"Test SPKS13: check descriptor bounds on subsetted file.",
        ctx,
    )?;
    ...
    // segment.
    //
    spicelib::DAFOPR(SPK13S, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
    ...
    // Load the SPK file.
    //
    spicelib::SPKLEF(SPK13S, &mut HANDLE, ctx)?;
    testutil::CHCKXC(false, b" ", OK, ctx)?;
