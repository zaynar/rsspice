...
pub fn SPKGEO_O(
    ...
    if (REFID == 0) {
        if (spicelib::FRSTNP(REF) > 0) {
            spicelib::SETMSG(b"The string supplied to specify the reference frame, (\'#\') contains non-printing characters.  The two most common causes for this kind of error are: 1. an error in the call to SPKGEO_O; 2. an uninitialized variable. ", ctx);
