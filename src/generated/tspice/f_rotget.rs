...
pub fn F_ROTGET(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    for I in 1..=18 {
        spicelib::ROTGET(I, ET, ROTAT.as_slice_mut(), &mut OUTFRM, &mut FOUND, ctx)?;
    ...
    testutil::TCASE(
        b"Verify that inertial class frames that are not yet supported are not found by ROTGET. ",
        ctx,
    )?;

    spicelib::ROTGET(99, ET, ROTAT.as_slice_mut(), &mut OUTFRM, &mut FOUND, ctx)?;
    ...
    testutil::TSTATD(ET, TMPROT.as_slice_mut(), AV.as_slice_mut(), ctx);
    spicelib::XPOSE(TMPROT.as_slice(), ROT.as_slice_mut());
    ...
    for I in 1..=3 {
        ...
        spicelib::ROTGET(ID, ET, ROTAT.as_slice_mut(), &mut OUTFRM, &mut FOUND, ctx)?;
