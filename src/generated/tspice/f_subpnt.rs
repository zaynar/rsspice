...
const CORLEN: i32 = 5;
const DSK0: &[u8] = b"subpnt_dsk0.bds";
const DSK1: &[u8] = b"subpnt_dsk1.bds";
const DSK2: &[u8] = b"subpnt_dsk2.bds";
const DSK3: &[u8] = b"subpnt_dsk3.bds";
const DSK4: &[u8] = b"subpnt_dsk4.bds";
const PCK: &[u8] = b"test_0008.tpc";
const SPK1: &[u8] = b"subpnt_spk.bsp";
...
pub fn F_SUBPNT(OK: &mut bool, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    //
    // Create DSK for Mars with coverage shifted from the normal
    // range.
    //
    // Use surface 3.
    //
    ...
    save.FIRST = spicelib::JYEAR();
    save.LAST = (spicelib::JYEAR() * 2 as f64);
    ...
    //***********************************************************************
    //
    //     Normal case: DSK time bounds don't include J2000.
    //
    //***********************************************************************
    ...
    //
    testutil::TCASE(
        b"Use DSK that doesn\'t include J2000 epoch in its coverage.",
        ctx,
    )?;
    ...
    //
    // Verify that no DSK data are available.
    //
    save.ET = (1.5 * spicelib::JYEAR());
    ...
    //
    // Make a call that requires data available at a TDB time not equal
    // to J2000.
    //
    ...
    //
    // Unload DSK4 so as to not interfere with later tests.
    //
    spicelib::UNLOAD(DSK4, ctx)?;

    //
    // Restore previously loaded DSKs.
    //
    spicelib::FURNSH(DSK0, ctx)?;
    spicelib::FURNSH(DSK1, ctx)?;
    spicelib::FURNSH(DSK2, ctx)?;
    ...
    // by SUBPNT. The subsystems (or structures) having states that must
    // be monitored are:
    //
