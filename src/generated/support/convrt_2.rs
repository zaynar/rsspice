...
impl SaveInit for SaveVars {
    fn new() -> Self {
        ...
        let mut NAME = ActualCharArray::new(8, 1..=MAXDIM);
...
pub fn CONVRT_2(
    ...
    if !UNITP(UNIN, ctx)? {
        spicelib::SETMSG(
            b"The input unit, #, was not recognized as a valid unit specification.",
            ctx,
        );
        spicelib::ERRCH(b"#", UNIN, ctx);
        spicelib::SIGERR(b"SPICE(UNKNOWNUNITS)", ctx)?;
        spicelib::CHKOUT(b"CONVRT_2", ctx)?;
        return Ok(());
    }

    if !UNITP(UNOUT, ctx)? {
        spicelib::SETMSG(
            b"The output unit, #, was not recognized as a valid unit specification.",
            ctx,
        );
        spicelib::ERRCH(b"#", UNIN, ctx);
        spicelib::SIGERR(b"SPICE(UNKNOWNUNITS)", ctx)?;
    ...
    for I in 1..=NDIM {
        if (save.DIMENI[I] != save.DIMENO[I]) {
            ...
            spicelib::SETMSG(b"The input and output units are not dimensionally equivalent.  The difference between the input and output dimension for # is #.", ctx);

            spicelib::ERRCH(b"#", &save.NAME[I], ctx);
            spicelib::ERRDP(b"#", save.DIFF, ctx);
            spicelib::SIGERR(b"SPICE(NOTDIMENSIONALLYEQUIV)", ctx)?;
            spicelib::CHKOUT(b"CONVRT_2", ctx)?;
            return Ok(());
        }
    }
