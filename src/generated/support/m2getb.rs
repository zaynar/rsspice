...
pub fn M2GETB(
    ...
    //
    M2BODN2C(fstr::substr(STRING, B..=E), &mut MYINT, FOUND, ctx)?;
    ...
    if !*FOUND {
        ...
        if fstr::ne(&ERROR, b" ") {
            spicelib::CHKIN(b"M2GETB", ctx)?;
            spicelib::SETMSG(b"The item requested, \'#\',  was not a recognized body and could not be parsed as an integer. ", ctx);
            spicelib::ERRCH(b"#", fstr::substr(STRING, B..=E), ctx);

            spicelib::SIGERR(b"META/2(CORRUPTEDBODYNAME)", ctx)?;
