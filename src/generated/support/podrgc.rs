...
pub fn PODRGC(POD: CharArrayMut, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    if (OFFSET != 0) {
        ...
        DCODEC(&POD[OFFSET], &mut NEWOFF, ctx)?;

        for I in 1..=NUMBER {
            let val = POD.get((OFFSET + I)).to_vec();
            fstr::assign(POD.get_mut((NEWOFF + I)), &val);
        }

        spicelib::SCARDC((NEWOFF + NUMBER), POD.as_arg_mut(), ctx)?;
    }

    spicelib::CHKOUT(b"PODRGC", ctx)?;
    Ok(())
}
