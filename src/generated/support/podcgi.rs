...
pub fn PODCGI(POD: &mut [i32], ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    if (OFFSET != 0) {
        ...
        spicelib::SCARDI(((OFFSET + NUMBER) - 1), POD.as_slice_mut(), ctx)?;
    }

    spicelib::CHKOUT(b"PODCGI", ctx)?;
    Ok(())
}
