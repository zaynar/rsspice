...
pub fn PODEGC(POD: CharArrayMut, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    if (OFFSET == 0) {
        spicelib::SCARDC(0, POD.as_arg_mut(), ctx)?;
    } else {
        let val = POD.get(OFFSET).to_vec();
        fstr::assign(POD.get_mut(GRPOFF), &val);
        spicelib::SCARDC((OFFSET - 1), POD.as_arg_mut(), ctx)?;
    }

    spicelib::CHKOUT(b"PODEGC", ctx)?;
    Ok(())
}
