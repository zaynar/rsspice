...
pub fn PODCGD(POD: &mut [f64], ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    if (OFFSET != 0) {
        ...
        spicelib::SCARDD(((OFFSET + NUMBER) - 1), POD.as_slice_mut(), ctx)?;
    }

    spicelib::CHKOUT(b"PODCGD", ctx)?;
    Ok(())
}
