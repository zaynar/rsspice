...
//
pub fn NSPSAV(FILE: &[u8], ERROR: CharArrayMut, ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    //
    NSPOPN(b"SAVE", FILE, ctx)?;

    if HAVE(ERROR.as_arg_mut(), ctx)? {
        spicelib::PREFIX(b"NSPSAV: ", 1, &mut ERROR[1]);
        return Ok(());
    }
