...
pub fn PODAEI(ELEMS: &[i32], N: i32, POD: &mut [i32], ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    // If not, bail out.
    //
    END = spicelib::CARDI(POD.as_slice(), ctx)?;

    if (spicelib::SIZEI(POD.as_slice(), ctx)? < (END + N)) {
        spicelib::SETMSG(b"Cannot fit # elements into # spaces.", ctx);
        spicelib::ERRINT(b"#", N, ctx);
        spicelib::ERRINT(b"#", (spicelib::SIZEI(POD.as_slice(), ctx)? - END), ctx);
    ...
    //
        ...
        spicelib::SCARDI((END + N), POD.as_slice_mut(), ctx)?;
    }
