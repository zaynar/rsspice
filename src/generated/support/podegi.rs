...
pub fn PODEGI(POD: &mut [i32], ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    if (OFFSET == 0) {
        spicelib::SCARDI(0, POD.as_slice_mut(), ctx)?;
    } else {
        POD[GRPOFF] = POD[OFFSET];
        spicelib::SCARDI((OFFSET - 1), POD.as_slice_mut(), ctx)?;
    }

    spicelib::CHKOUT(b"PODEGI", ctx)?;
    Ok(())
}
