...
pub fn PODRGD(POD: &mut [f64], ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    if (OFFSET != 0) {
        ...
        DCODED(POD[OFFSET], &mut NEWOFF, ctx)?;

        for I in 1..=NUMBER {
            POD[(NEWOFF + I)] = POD[(OFFSET + I)];
        }

        spicelib::SCARDD((NEWOFF + NUMBER), POD.as_slice_mut(), ctx)?;
    }

    spicelib::CHKOUT(b"PODRGD", ctx)?;
    Ok(())
}
