...
/// ```
pub fn pl2nvc(plane: &[f64; 4], normal: &mut [f64; 3], konst: &mut f64) {
    PL2NVC(plane, normal, konst);
}

//$Procedure PL2NVC ( Plane to normal vector and constant )
pub fn PL2NVC(PLANE: &[f64], NORMAL: &mut [f64], KONST: &mut f64) {
    ...
    // Unpack the plane.
    //
    VEQU(PLANE.subarray(NMLPOS), NORMAL.as_slice_mut());
    *KONST = PLANE[CONPOS];
}
