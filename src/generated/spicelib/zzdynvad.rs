...
pub fn ZZDYNVAD(
    ...
    if (REQNUM <= KVNMLN) {
        ...
        DTPOOL(&KVNAME, &mut FOUND, N, &mut DTYPE, ctx)?;
    } else {
        //
    ...
    if fstr::eq(&DTYPE, b"C") {
        SETMSG(b"The kernel variable # has used to define frame # was expected to have double precision data type but in fact has  character type.  Usually this type of problem is due to an error in a frame definition provided in a frame kernel.", ctx);
        ERRCH(b"#", &KVNAME, ctx);
        ERRCH(b"#", FRNAME, ctx);
    ...
    //
    GDPOOL(&KVNAME, 1, MAXN, N, VALUES.as_slice_mut(), &mut FOUND, ctx)?;
