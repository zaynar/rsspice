...
pub fn REPMF(
    ...
    let mut GDFMT = [b' '; 1 as usize];
    let mut SUBSTR = [b' '; MAXLFD as usize];
    ...
    //
    LJUST(FORMAT, &mut GDFMT);
    UCASE(&GDFMT.clone(), &mut GDFMT, ctx);

    DPSTRF(VALUE, SIGDIG, &GDFMT, &mut SUBSTR, ctx);
