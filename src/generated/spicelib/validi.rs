...
///         removed, also.
/// ```
pub fn validi(ctx: &mut SpiceContext, size: i32, n: i32, a: &mut [i32]) -> crate::Result<()> {
    VALIDI(size, n, a, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(())
}

//$Procedure VALIDI ( Validate an integer set )
pub fn VALIDI(SIZE: i32, N: i32, A: &mut [i32], ctx: &mut Context) -> f2rust_std::Result<()> {
    ...
    //
    CARD = N;
    RMDUPI(&mut CARD, A.subarray_mut(1));

    //
    // Set the size and cardinality of the input set.
    //
    SSIZEI(SIZE, A.as_slice_mut(), ctx)?;
    SCARDI(CARD, A.as_slice_mut(), ctx)?;
