...
/// ```
pub fn seti(ctx: &mut SpiceContext, a: &[i32], op: &str, b: &[i32]) -> crate::Result<bool> {
    let ret = SETI(a, op.as_bytes(), b, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(ret)
}

//$Procedure SETI ( Compare integer sets )
pub fn SETI(A: &[i32], OP: &[u8], B: &[i32], ctx: &mut Context) -> f2rust_std::Result<bool> {
    let A = DummyArray::new(A, LBCELL..);
    let B = DummyArray::new(B, LBCELL..);
    let mut SETI: bool = false;
    let mut CARDA: i32 = 0;
    let mut CARDB: i32 = 0;
    ...
    //
    CARDA = CARDI(A.as_slice(), ctx)?;
    CARDB = CARDI(B.as_slice(), ctx)?;
