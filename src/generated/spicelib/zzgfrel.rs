...
pub fn ZZGFREL(
    ...
    UDQDEC: fn(f64, &mut bool, &mut Context) -> f2rust_std::Result<()>,
    UDCOND: fn(f64, &mut bool, &mut Context) -> f2rust_std::Result<()>,
    UDFUNC: fn(f64, &mut f64, &mut Context) -> f2rust_std::Result<()>,
    UDQREF: fn(f64, &mut Context) -> (),
    RELATE: &[u8],
    REFVAL: f64,
    TOL: f64,
    ADJUST: f64,
    ...
    if (fstr::eq(&LOCREL, b"ABSMIN") || fstr::eq(&LOCREL, b"ABSMAX")) {
        ...
        for CASE in 1..=2 {
            ...
            for I in 1..=WINSIZ {
                ...
                UDFUNC(ENDPT[WANT], &mut VALUE, ctx)?;
