...
/// ```
pub fn sizei(ctx: &mut SpiceContext, cell: &[i32]) -> crate::Result<i32> {
    let ret = SIZEI(cell, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(ret)
}

//$Procedure SIZEI ( Size of an integer cell )
pub fn SIZEI(CELL: &[i32], ctx: &mut Context) -> f2rust_std::Result<i32> {
