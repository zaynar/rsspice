...
pub fn ZZDYNFID(
    ...
    let mut OUTNAM = [b' '; FRNMLN as usize];
    ...
    if !FOUND {
        ...
        if (!FOUND && (REQNUM > KVNMLN)) {
            ...
            //
            SETMSG(b"At least one of the kernel variables FRAME_#_# or FRAME_#_# was expected to be present in the kernel pool but neither was found. One of these variables is needed to define the parameterized dynamic frame #.  Usually this type of problem is due to a missing keyword assignment in a frame kernel.  Another, less likely, possibility is that other errors in a frame kernel have confused the frame subsystem into wrongly deciding these variables are needed.", ctx);
            ERRINT(b"#", FRCODE, ctx);
            ERRCH(b"#", ITEM, ctx);
            ERRCH(b"#", FRNAME, ctx);
            ERRCH(b"#", ITEM, ctx);
    ...
    if fstr::eq(&DTYPE, b"C") {
        ...
        GCPOOL(
            ...
            1,
            &mut N,
            CharArrayMut::from_mut(&mut OUTNAM),
            &mut FOUND,
            ctx,
        )?;

        if !FOUND {
            SETMSG(b"The kernel variable # has used to define frame # was not found after DTPOOL indicated it was present in pool.", ctx);
            ERRCH(b"#", &KVNAME, ctx);
            ERRCH(b"#", FRNAME, ctx);
        ...
        if (*IDCODE == 0) {
            ...
            //
            if BEINT(&OUTNAM) {
                PRSINT(&OUTNAM, IDCODE, ctx)?;
            } else {
                //
                // We're outta aces.
                //
                SETMSG(b"The kernel variable # used to define frame # is assigned the character value #.  This value was expected to be a reference frame name, but NAMFRM cannot translate this name to a frame ID code.", ctx);
                ERRCH(b"#", &KVNAME, ctx);
                ERRCH(b"#", FRNAME, ctx);
                ERRCH(b"#", &OUTNAM, ctx);
