...
pub fn dcyldr(
    ...
    DCYLDR(x, y, z, jacobi.as_flattened_mut(), ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(())
}
...
pub fn DCYLDR(
    ...
    //
    if ((X == 0.0) && (Y == 0.0)) {
        SETMSG(b"The Jacobian of the transformation from rectangular to cylindrical coordinates is not defined for points on the z-axis.", ctx);
    ...
    //
    RECCYL(RECTAN.as_slice(), &mut R, &mut LONG, &mut ZZ, ctx);

    //
    // Get the Jacobian from cylindrical to rectangular coordinates at
    // R, LONG, Z.
    //
    DRDCYL(R, LONG, ZZ, INJACB.as_slice_mut());
