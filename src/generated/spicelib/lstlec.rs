...
/// ```
pub fn lstlec(string: &str, n: i32, array: CharArray) -> i32 {
    let ret = LSTLEC(string.as_bytes(), n, array);
    ret
}

//$Procedure LSTLEC ( Last character element less than or equal to. )
pub fn LSTLEC(STRING: &[u8], N: i32, ARRAY: CharArray) -> i32 {
    let ARRAY = DummyCharArray::new(ARRAY, None, 1..);
    let mut LSTLEC: i32 = 0;
    let mut J: i32 = 0;
    let mut BEGIN: i32 = 0;
    let mut END: i32 = 0;
    ...
    if (N <= 0) {
        ...
        //
        LSTLEC = 0;
    } else if fstr::lt(STRING, &ARRAY[BEGIN]) {
        //
        // None of the array elements are less than or equal to STRING
        //
        LSTLEC = 0;
    } else if fstr::ge(STRING, &ARRAY[END]) {
        //
        ...
        while (ITEMS > 2) {
            ...
            if fstr::le(&ARRAY[MIDDLE], STRING) {
                BEGIN = MIDDLE;
            } else {
                END = MIDDLE;
            }
