...
pub fn ZZEKAC07(
    ...
    if (CLASS != 7) {
        ...
        SETMSG(
            b"Column class code # found in descriptor for column #.  Class should be 7.",
            ctx,
        );
        ERRINT(b"#", CLASS, ctx);
        ERRCH(b"#", &COLUMN, ctx);
        SIGERR(b"SPICE(NOCLASS)", ctx)?;
        CHKOUT(b"ZZEKAC07", ctx)?;
        return Ok(());
    }
    ...
    DASUDI(HANDLE, (CMBASE + DBIX07), (CMBASE + DBIX07), &[DATBAS], ctx)?;
    ...
    if NULLOK {
        ...
        // array.
        //
        DASUDI(HANDLE, (CMBASE + NFIX07), (CMBASE + NFIX07), &[NFLBAS], ctx)?;
    }
