...
pub fn ZZEKRD04(
    ...
    if (DATPTR > 0) {
        ...
        *ISNULL = false;
        ...
        // can't be met.
        //
        DASRDI(HANDLE, DATPTR, DATPTR, std::slice::from_mut(&mut NELT), ctx)?;
        ...
        while (MAXIDX < BEG) {
            ...
            //
            ZZEKPGBS(INT, P, &mut BASE, ctx)?;

            MINIDX = (MAXIDX + 1);
            MAXIDX = intrinsics::MIN0(&[(MAXIDX + IPSIZE), NELT]);
        ...
        // read them.
        //
        NREAD = intrinsics::MIN0(&[REMAIN, (((BASE + IPSIZE) - DATPTR) + 1)]);

        DASRDI(
            HANDLE,
            DATPTR,
            ((DATPTR + NREAD) - 1),
            IVALS.subarray_mut(START),
            ctx,
        )?;
        ...
        while ((REMAIN > 0) && !FAILED(ctx)) {
            ...
            // Locate the page on which the element is continued.
            //
            ZZEKGFWD(HANDLE, INT, P, &mut PTEMP, ctx)?;
            P = PTEMP;
            ...
            DATPTR = (BASE + 1);
            START = (START + NREAD);
            NREAD = intrinsics::MIN0(&[REMAIN, IPSIZE]);
