...
pub fn ZZEKAC05(
    ...
    let mut ADRBUF = StackArray::<i32, 126>::new(1..=BUFSIZ);
    ...
    if (CLASS != 5) {
        ...
        SETMSG(
            b"Column class code # found in descriptor for column #.  Class should be 5.",
            ctx,
        );
        ERRINT(b"#", CLASS, ctx);
        ERRCH(b"#", &COLUMN, ctx);
        SIGERR(b"SPICE(NOCLASS)", ctx)?;
        CHKOUT(b"ZZEKAC05", ctx)?;
        return Ok(());
    }
