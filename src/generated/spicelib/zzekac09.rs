...
pub fn ZZEKAC09(
    ...
    if (CLASS != 9) {
        ...
        SETMSG(
            b"Column class code # found in descriptor for column #.  Class should be 9.",
            ctx,
        );
        ERRINT(b"#", CLASS, ctx);
        ERRCH(b"#", &COLUMN, ctx);
        SIGERR(b"SPICE(NOCLASS)", ctx)?;
        CHKOUT(b"ZZEKAC09", ctx)?;
        return Ok(());
    }
    ...
    if !FIXLEN {
        ...
        SETMSG(
            b"Column # has variable string length; class 9 supports fixed-length strings only.",
            ctx,
        );
        ERRCH(b"#", &COLUMN, ctx);
    ...
    DASUDI(HANDLE, (CMBASE + DBIX09), (CMBASE + DBIX09), &[DATBAS], ctx)?;
    ...
    if NULLOK {
        ...
        // array.
        //
        DASUDI(HANDLE, (CMBASE + NFIX09), (CMBASE + NFIX09), &[NFLBAS], ctx)?;
    }
