...
/// ```
pub fn wnfild(ctx: &mut SpiceContext, smlgap: f64, window: &mut [f64]) -> crate::Result<()> {
    WNFILD(smlgap, window, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(())
}

//$Procedure WNFILD ( Fill small gaps in a DP window )
pub fn WNFILD(SMLGAP: f64, WINDOW: &mut [f64], ctx: &mut Context) -> f2rust_std::Result<()> {
