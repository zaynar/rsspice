...
/// ```
pub fn setd(ctx: &mut SpiceContext, a: &[f64], op: &str, b: &[f64]) -> crate::Result<bool> {
    let ret = SETD(a, op.as_bytes(), b, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(ret)
}

//$Procedure SETD ( Compare double precision sets )
pub fn SETD(A: &[f64], OP: &[u8], B: &[f64], ctx: &mut Context) -> f2rust_std::Result<bool> {
    let A = DummyArray::new(A, LBCELL..);
    let B = DummyArray::new(B, LBCELL..);
    let mut SETD: bool = false;
    let mut CARDA: i32 = 0;
    let mut CARDB: i32 = 0;
    ...
    //
    CARDA = CARDD(A.as_slice(), ctx)?;
    CARDB = CARDD(B.as_slice(), ctx)?;
