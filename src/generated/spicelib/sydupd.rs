...
pub fn SYDUPD(
    ...
    if !OLDSYM[1] {
        SETMSG(
            b"SYDUPD: The symbol to be duplicated, #, is not in the symbol table.",
            ctx,
        );
        ERRCH(b"#", NAME, ctx);
    ...
    //
        ...
        if ((NSYM + NEWSYM) > SIZEC(TABSYM.as_arg(), ctx)?) {
            SETMSG(
                b"SYDUPD: Duplication of the symbol # causes an overflow in the name table.",
                ctx,
            );
            ERRCH(b"#", NAME, ctx);
            ...
            SETMSG(
                b"SYDUPD: Duplication of the symbol # causes an overflow in the pointer table.",
                ctx,
            );
            ERRCH(b"#", NAME, ctx);
            ...
            SETMSG(
                b"SYDUPD: Duplication of the symbol # causes an overflow in the value table.",
                ctx,
            );
            ERRCH(b"#", NAME, ctx);
        ...
        //
            ...
            //
            if (DIMVAL[2] > 0) {
                REMLAD(DIMVAL[2], LOCVAL[2], TABVAL.subarray_mut(1), &mut NVAL, ctx)?;
            ...
            SCARDD((NVAL + DIMVAL[1]), TABVAL.as_slice_mut(), ctx)?;
        }
    }
