...
///         removed, also.
/// ```
pub fn ssizei(ctx: &mut SpiceContext, size: i32, cell: &mut [i32]) -> crate::Result<()> {
    SSIZEI(size, cell, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(())
}

//$Procedure SSIZEI ( Set the size of an integer cell )
pub fn SSIZEI(SIZE: i32, CELL: &mut [i32], ctx: &mut Context) -> f2rust_std::Result<()> {
