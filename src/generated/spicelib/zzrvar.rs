...
impl SaveInit for SaveVars {
    fn new() -> Self {
        ...
        let mut ERROR = vec![b' '; 255 as usize];
...
pub fn ZZRVAR(
    ...
    while ((save.STATUS != DONE) && !FAILED(ctx)) {
        ...
        if (save.STATUS == BEGIN) {
            ...
            //
            if (save.COUNT < 3) {
                RDKLIN(&mut save.FILE, &mut save.NUMBER, ctx);
                SETMSG(b"A kernel variable was not properly formed on line # of the file #. Such an assignment should have the form: \'<variable name> [+]= <values>\'. This line was \'#\'. ", ctx);

                save.R1 = RTRIM(&save.FILE);
                save.R2 = RTRIM(&save.LINE);

                ERRINT(b"#", save.NUMBER, ctx);
                ERRCH(b"#", fstr::substr(&save.FILE, 1..=save.R1), ctx);
            ...
            if (save.BADAT <= (save.ENDS[1] - save.BEGS[1])) {
                ...
                SETMSG(b"There is a non-printing character embedded in line # of the text kernel file #.  Non-printing characters are not allowed in kernel variable assignments.  The non-printing character has ASCII code #. ", ctx);
            ...
            if (save.VARLEN > MAXLEN) {
                SETMSG(b"A kernel pool variable name read from a kernel file exceeds the maximum allowed length #1. The actual length of the variable name is #2, the offending variable name to #3 characters: \'#4\'.", ctx);
            ...
            if ISBAD(save.DIRCTV) {
                RDKLIN(&mut save.FILE, &mut save.NUMBER, ctx);
                SETMSG(b"A kernel variable was not properly formed on line # of the file #. Such an assignment should have the form: \'<variable name> [+]= <values>\'.  More specifically, the assignment operator did not have one of the expected forms: \'=\' or \'+=\'. The line was \'#\'. ", ctx);
            ...
            if (save.FULL && !save.FOUND) {
                ...
                SETMSG(b"The kernel pool does not have room for any more variables.  It filled up at line # of the kernel file #. ", ctx);
        ...
        while ((save.TYPE[save.NXTTOK] != EV) && (save.NXTTOK <= save.COUNT)) {
            ...
            if (save.VARTYP == UNKNWN) {
                ...
                if (save.TYPE[save.NXTTOK] == Q) {
                    ...
                    SETMSG(b"The first item following the assignment operator should be the value of a variable or a left parenthesis \'(\' followed by a value for a variable. This is not true on line # of the text kernel file \'#\'. ", ctx);
            ...
            if (save.VARTYP == STRTYP) {
                ...
                if (save.TYPE[save.NXTTOK] != Q) {
                    ...
                    save.R2 = RTRIM(&save.FILE);

                    SETMSG(b"The kernel variable # has been set up as a string variable.  However, the value that you are attempting to assign to this variable on line # of the kernel file \'#\' is not a string value. ", ctx);

                    ERRCH(b"#", fstr::substr(VARNAM, 1..=save.R1), ctx);
                    ERRINT(b"#", save.NUMBER, ctx);
                    ERRCH(b"#", fstr::substr(&save.FILE, 1..=save.R2), ctx);
                ...
                if ((save.B + 1) >= save.E) {
                    ...
                    SETMSG(b"There is a quoted string with no characters on line # of the text kernel file \'#\'. ", ctx);
                ...
                if (save.FREE <= 0) {
                    ...
                    SETMSG(b"There is no room available for adding another character value to the kernel pool.  The character values buffer became full at line # of the text kernel file \'#\'. ", ctx);
                ...
                //
                save.CODE = intrinsics::ICHAR(fstr::substr(&save.LINE, save.E..=save.E));
                ...
                while (save.J < save.E) {
                    ...
                    if (save.I <= intrinsics::LEN(&CHVALS[save.CHNODE])) {
                        fstr::assign(
            ...
            //
                ...
                if (save.TYPE[save.NXTTOK] != NQ) {
                    ...
                    SETMSG(b"The kernel variable # has been set up as a numeric or time variable.  However, the value that you are attempting to assign to this variable on line # of the kernel file \'#\' is not a numeric or time value. ", ctx);
                ...
                if ISTIME(save.CODE, save.ITMARK) {
                    ...
                    if (save.E == save.B) {
                        ...
                        SETMSG(b"At character # of  line # in the text kernel file \'#\' the character \'@\' appears.  This character is reserved for identifying time values in assignments to kernel pool variables.  However it is not being used in this fashion for the variable \'#\'. ", ctx);
                    ...
                    if fstr::ne(&save.ERROR, b" ") {
                        ...
                        save.LSTNB = LASTNB(&save.ERROR);

                        SETMSG(b"Encountered \'#\' while attempting to parse a time on line # of the text kernel file \'#\'.  Error message: \'#\'", ctx);

                        ERRCH(b"#", fstr::substr(&save.LINE, (save.B + 1)..=save.E), ctx);
                        ERRINT(b"#", save.NUMBER, ctx);
                        ERRCH(b"#", &save.FILE, ctx);
                        ERRCH(b"#", fstr::substr(&save.ERROR, 1..=save.LSTNB), ctx);
                        SIGERR(b"SPICE(BADTIMESPEC)", ctx)?;
                        CHKOUT(b"ZZRVAR", ctx)?;
                        return Ok(());
                    }
                    ...
                    if fstr::ne(&save.ERROR, b" ") {
                        ...
                        SETMSG(b"Encountered \'#\' while attempting to parse a number on line # of the text kernel file \'#\'.  Error message: \'#\'", ctx);
                ...
                if (save.FREE <= 0) {
                    ...
                    SETMSG(b"There is no room available for adding another numeric value to the kernel pool.  The numeric values buffer became full at line # of the text kernel file \'#\'. ", ctx);
