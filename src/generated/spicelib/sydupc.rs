...
pub fn SYDUPC(
    ...
    if !OLDSYM[1] {
        SETMSG(
            b"SYDUPC: The symbol to be duplicated, #, is not in the symbol table.",
            ctx,
        );
        ERRCH(b"#", NAME, ctx);
    ...
    //
        ...
        if ((NSYM + NEWSYM) > SIZEC(TABSYM.as_arg(), ctx)?) {
            SETMSG(
                b"SYDUPC: Duplication of the symbol # causes an overflow in the name table.",
                ctx,
            );
            ERRCH(b"#", NAME, ctx);
            ...
            SETMSG(
                b"SYDUPC: Duplication of the symbol # causes an overflow in the pointer table.",
                ctx,
            );
            ERRCH(b"#", NAME, ctx);
            SIGERR(b"SPICE(POINTERTABLEFULL)", ctx)?;
        } else if ((NVAL + NEWVAL) > SIZEC(TABVAL.as_arg(), ctx)?) {
            SETMSG(
                b"SYDUPC: Duplication of the symbol # causes an overflow in the value table.",
                ctx,
            );
            ERRCH(b"#", NAME, ctx);
        ...
        //
            ...
            //
            if (DIMVAL[2] > 0) {
                REMLAC(DIMVAL[2], LOCVAL[2], TABVAL.subarray_mut(1), &mut NVAL, ctx)?;
            ...
            for I in intrinsics::range(NVAL, LOCVAL[2], -1) {
                let val = TABVAL.get(I).to_vec();
                fstr::assign(TABVAL.get_mut((I + DIMVAL[1])), &val);
            }
            ...
            for I in 0..=(DIMVAL[1] - 1) {
                let val = TABVAL.get((LOCVAL[1] + I)).to_vec();
                fstr::assign(TABVAL.get_mut((LOCVAL[2] + I)), &val);
            }

            SCARDC((NVAL + DIMVAL[1]), TABVAL.as_arg_mut(), ctx)?;
        }
    }
