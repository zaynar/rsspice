...
pub fn dasiod(
    ...
    DASIOD(action.as_bytes(), unit, recno, record, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(())
}
...
pub fn DASIOD(
    ...
    if EQSTR(ACTION, b"READ") {
        ...
        if (IOSTAT != 0) {
            CHKIN(b"DASIOD", ctx)?;
            SETMSG(b"Could not read DAS double precision record. File = # Record number = #. IOSTAT = #.", ctx);
            ERRFNM(b"#", UNIT, ctx)?;
            ERRINT(b"#", RECNO, ctx);
            ERRINT(b"#", IOSTAT, ctx);
        ...
        if (IOSTAT != 0) {
            CHKIN(b"DASIOD", ctx)?;
            SETMSG(b"Could not write DAS double precision record. File = # Record number = #. IOSTAT = #.", ctx);
            ERRFNM(b"#", UNIT, ctx)?;
            ERRINT(b"#", RECNO, ctx);
            ERRINT(b"#", IOSTAT, ctx);
