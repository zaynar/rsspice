...
pub fn getelm(
    ...
    GETELM(frstyr, lines, epoch, elems, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(())
}
...
pub fn GETELM(
    ...
    let mut ELEMS = DummyArrayMut::new(ELEMS, 1..=10);
    let mut OK: bool = false;
    let mut ERROR = [b' '; 256 as usize];
    ...
    //
    if !OK {
        SETMSG(b"Error in TLE set. #", ctx);
        ERRCH(b"#", &ERROR, ctx);
        SIGERR(b"SPICE(BADTLE)", ctx)?;
        CHKOUT(b"GETELM", ctx)?;
        return Ok(());
    }
