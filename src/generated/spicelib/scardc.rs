...
/// ```
pub fn scardc(ctx: &mut SpiceContext, card: i32, cell: CharArrayMut) -> crate::Result<()> {
    SCARDC(card, cell, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(())
}

//$Procedure SCARDC ( Set the cardinality of a character cell )
pub fn SCARDC(CARD: i32, CELL: CharArrayMut, ctx: &mut Context) -> f2rust_std::Result<()> {
