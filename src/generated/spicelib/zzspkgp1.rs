...
pub fn ZZSPKGP1(
    ...
    if (REFID == 0) {
        if (FRSTNP(REF) > 0) {
            SETMSG(b"The string supplied to specify the reference frame, (\'#\') contains non-printing characters.  The two most common causes for this kind of error are: 1. an error in the call to ZZSPKGP1; 2. an uninitialized variable. ", ctx);
            ERRCH(b"#", REF, ctx);
        ...
        while ((FOUND && (CTARG[CHLEN] != SSB)) && (CTARG[CHLEN] != OBS)) {
            ...
            if FOUND {
                ...
                if (TFRAME[CHLEN] == TMPFRM) {
                    ...
                    ZZREFCH1(TFRAME[CHLEN], TMPFRM, ET, PSXFRM.as_slice_mut(), ctx)?;
    ...
    while ((FOUND && (COBS != SSB)) && (CTPOS == 0)) {
        ...
        if FOUND {
            ...
            if (CFRAME == TMPFRM) {
                ...
                ZZREFCH1(CFRAME, TMPFRM, ET, PSXFRM.as_slice_mut(), ctx)?;
...

    {
        ...
        for _ in 0..((m2__ - m1__ + m3__) / m3__) as i32 {
            if (TFRAME[I] == TFRAME[(I + 1)]) {
                ...
                ZZREFCH1(TFRAME[I], TFRAME[(I + 1)], ET, PSXFRM.as_slice_mut(), ctx)?;
    ...
    if (TFRAME[CTPOS] == CFRAME) {
        ...
        if ISINRT(CFRAME, REFID) {
            ...
            ZZREFCH1(CFRAME, REFID, ET, PSXFRM.as_slice_mut(), ctx)?;
        ...
        // transformation.
        //
        ZZREFCH1(TFRAME[CTPOS], CFRAME, ET, PSXFRM.as_slice_mut(), ctx)?;
