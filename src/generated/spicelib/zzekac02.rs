...
pub fn ZZEKAC02(
    ...
    if (CLASS != 2) {
        ...
        SETMSG(
            b"Column class code # found in descriptor for column #.  Class should be 2.",
            ctx,
        );
        ERRINT(b"#", CLASS, ctx);
        ERRCH(b"#", &COLUMN, ctx);
        SIGERR(b"SPICE(NOCLASS)", ctx)?;
        CHKOUT(b"ZZEKAC02", ctx)?;
        return Ok(());
    }
    ...
    while (REMAIN > 0) {
        ...
        if ((N == DPSIZE) || ((NWRITE == NDATA) && (NDATA != 0))) {
            //
