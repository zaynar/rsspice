...
pub fn ZZEKTRSZ(HANDLE: i32, TREE: i32, ctx: &mut Context) -> f2rust_std::Result<i32> {
    let mut ZZEKTRSZ: i32 = 0;
    ...
    //
    ADDRSS = (ZZEKTRBS(TREE, ctx)? + TRNKEY);

    DASRDI(
        HANDLE,
        ADDRSS,
        ADDRSS,
        std::slice::from_mut(&mut ZZEKTRSZ),
        ctx,
    )?;
