...
pub fn ZZSPKGO1(
    ...
    if (REFID == 0) {
        if (FRSTNP(REF) > 0) {
            SETMSG(b"The string supplied to specify the reference frame, (\'#\') contains non-printing characters.  The two most common causes for this kind of error are: 1. an error in the call to ZZSPKGO1; 2. an uninitialized variable. ", ctx);
            ERRCH(b"#", REF, ctx);
        ...
        while ((FOUND && (CTARG[CHLEN] != SSB)) && (CTARG[CHLEN] != OBS)) {
            ...
            if FOUND {
                ...
                if (TFRAME[CHLEN] == TMPFRM) {
                    ...
                    ZZFRMCH1(TFRAME[CHLEN], TMPFRM, ET, STXFRM.as_slice_mut(), ctx)?;
    ...
    while ((FOUND && (COBS != SSB)) && (CTPOS == 0)) {
        ...
        if FOUND {
            ...
            if (CFRAME == TMPFRM) {
                ...
                ZZFRMCH1(CFRAME, TMPFRM, ET, STXFRM.as_slice_mut(), ctx)?;
...

    {
        ...
        for _ in 0..((m2__ - m1__ + m3__) / m3__) as i32 {
            if (TFRAME[I] == TFRAME[(I + 1)]) {
                ...
                ZZFRMCH1(TFRAME[I], TFRAME[(I + 1)], ET, STXFRM.as_slice_mut(), ctx)?;
    ...
    if (TFRAME[CTPOS] == CFRAME) {
        ...
        if ISINRT(CFRAME, REFID) {
            ...
            ZZFRMCH1(CFRAME, REFID, ET, STXFRM.as_slice_mut(), ctx)?;
        ...
        // transformation.
        //
        ZZFRMCH1(TFRAME[CTPOS], CFRAME, ET, STXFRM.as_slice_mut(), ctx)?;
