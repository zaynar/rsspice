...
pub fn ZZEKDE05(
    ...
    if (DATPTR > 0) {
        ...
        NELTS = intrinsics::IDNINT(DPNELT);
        ...
        //
        ZZEKGFWD(HANDLE, DP, P, &mut NEXT, ctx)?;
        ...
        if (NLINKS > 1) {
            ...
            //
            ZZEKDPS(HANDLE, SEGDSC.as_slice_mut(), DP, P, ctx)?;
        }

        NSEEN = intrinsics::MIN0(&[NELTS, ((BASE + DPSIZE) - DATPTR)]);
        ...
        while ((NSEEN < NELTS) && !FAILED(ctx)) {
            ...
            NSEEN = intrinsics::MIN0(&[NELTS, (DPSIZE + NSEEN)]);
        }
    } else if (DATPTR == NULL) {
        //
