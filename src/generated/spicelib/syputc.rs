...
pub fn SYPUTC(
    ...
    if (N < 1) {
        SETMSG(
            b"SYPUTC: The dimension of the values array isless than one.",
            ctx,
        );
    ...
    if ((NSYM + NEWSYM) > SIZEC(TABSYM.as_arg(), ctx)?) {
        SETMSG(
            b"SYPUTC: Addition of the new symbol # causes an overflow in the name table.",
            ctx,
        );
        ERRCH(b"#", NAME, ctx);
        ...
        SETMSG(
            b"SYPUTC: Addition of the new symbol # causes an overflow in the pointer table.",
            ctx,
        );
        ERRCH(b"#", NAME, ctx);
