...
///         removed, also.
/// ```
pub fn ssized(ctx: &mut SpiceContext, size: i32, cell: &mut [f64]) -> crate::Result<()> {
    SSIZED(size, cell, ctx.raw_context())?;
    ctx.handle_errors()?;
    Ok(())
}

//$Procedure SSIZED ( Set the size of a double precision cell )
pub fn SSIZED(SIZE: i32, CELL: &mut [f64], ctx: &mut Context) -> f2rust_std::Result<()> {
